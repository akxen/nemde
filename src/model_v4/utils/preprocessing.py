...
def get_trader_fcas_availability_status(data) -> dict:
    ...
    for (trader_id, trade_type), params in fcas_info.items():
        ...
        fcas_status[(trader_id, trade_type)] = fcas2.get_trader_fcas_availability_status(params)
...
if __name__ == '__main__':
    ...
    # Re-formatted case file
    t0 = time.time()
    intervention_status = get_intervention_status(cdata, 'physical')
    formatted_case = transforms.simplified.case.construct_case(cdata, intervention_status)
    print(time.time() - t0)
