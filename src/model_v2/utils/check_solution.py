...
def get_region_net_export(data, region_id):
    """Get region net export"""
    pass


def check_region_net_export(data):
    """Check region net export"""
    pass


def check_region_net_export_sample(data):
    """Check region net export for a random sample of dispatch intervals"""
    pass
...
def check_total_fixed_demand_calculation(data):
    ...
    for i in interconnectors:
        ...
        if mnsp_status == '1':
            ...
            # From region exporting, To region importing
            if flow >= 0:
                to_lf = lookup.get_interconnector_collection_attribute(data, i, '@ToRegionLFImport', float)
                from_lf = lookup.get_interconnector_collection_attribute(data, i, '@FromRegionLFExport', float)

                from_export_flow = abs(flow)
                to_import_flow = abs(flow)

                # TODO: not sure why need to subtract loss when flow is negative
                total_mnsp_loss -= (to_import_flow * (1 - to_lf)) + (from_export_flow * (1 - from_lf))

            else:
                to_lf = lookup.get_interconnector_collection_attribute(data, i, '@ToRegionLFExport', float)
                from_lf = lookup.get_interconnector_collection_attribute(data, i, '@FromRegionLFImport', float)

                from_import_flow = abs(flow)
                to_export_flow = abs(flow)

                # TODO: not sure why need to subtract loss when flow is negative
                total_mnsp_loss += (to_export_flow * (1 - to_lf)) + (from_import_flow * (1 - from_lf))
...
if __name__ == '__main__':
    ...
    # c1, c1_df, c1_max_diff = check_total_cleared_demand_calculation_sample(data_directory, n=100)
    # c2, c2_df, c2_max_diff = check_total_fixed_demand_calculation_sample(data_directory, n=100)
    c3, c3_df, c3_max_diff = check_region_net_export_calculation_sample(data_directory, n=100)
    # check_region_net_export_calculation(cdata, 'VIC1')
