...
def joint_ramp_raise_generator_rule(m, i):
    """
    ...
    # Check if unit has raise regulation offer
    ...
    return (m.V_TRADER_TOTAL_OFFER[i, 'ENOF'] + m.V_TRADER_TOTAL_OFFER[i, 'R5RE']
            <= m.P_TRADER_INITIAL_MW[i] + scada_ramp + m.V_CV_TRADER_FCAS_JOINT_RAMPING_RAISE_GENERATOR[i])


def joint_ramp_lower_generator_rule(m, i):
    """
    ...
    # Check if unit has lower regulation offer
...
def joint_ramp_raise_load_rule(m, i):
    ...
    return (m.V_TRADER_TOTAL_OFFER[i, 'LDOF'] - m.V_TRADER_TOTAL_OFFER[i, 'R5RE']
            + m.V_CV_TRADER_FCAS_JOINT_RAMPING_RAISE_LOAD[i] >= m.P_TRADER_INITIAL_MW[i] - scada_ramp)


def joint_ramp_lower_load_rule(m, i):
    """
...
def define_fcas_constraints(m):
    ...
    m.C_JOINT_RAMP_RAISE_GENERATOR = pyo.Constraint(m.S_TRADERS, rule=joint_ramp_raise_generator_rule)
